...
NMEAMessage.prototype.getElement = function(elementID, returnType) {
	if (elementID < 1 || elementID >= this.messageParts.length) {
		throw new RangeError("The element id [" + elementID + "] is not within the allowed range (1, " + (this.messageParts.length - 1) + ")!");
	}
	switch (returnType) {
		case 'string' :
		...
		default :
			throw new TypeError("Unknown returntype [" + returnType + "]!");
	}
}
